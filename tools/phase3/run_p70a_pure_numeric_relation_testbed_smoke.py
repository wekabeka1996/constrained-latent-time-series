...
def main():
    if len(sys.argv) > 1:
        print("Error: run_p70a_pure_numeric_relation_testbed_smoke.py does not accept command line arguments.", file=sys.stderr)
        sys.exit(1)
    ...
    probe_res = run_p70a_pure_numeric_relation_testbed_probe()
    
    compact_json = json.dumps(probe_res, sort_keys=True, separators=(",", ":"))
    print(compact_json)
    
    if probe_res.get("verdict") == "P70A_READY_FOR_REVIEW":
        sys.exit(0)
    else:
        sys.exit(1)
