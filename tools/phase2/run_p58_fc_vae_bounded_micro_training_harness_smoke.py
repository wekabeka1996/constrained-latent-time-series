...
def main():
    if len(sys.argv) > 1:
        print("Error: run_p58_fc_vae_bounded_micro_training_harness_smoke.py does not accept command line arguments.", file=sys.stderr)
        sys.exit(1)
    ...
    probe_res = run_fc_vae_bounded_micro_training_harness_probe()
    
    final_dict = fc_vae_bounded_micro_training_harness_probe_to_json_dict(probe_res)
    
    final_dict["verdict"] = "PASS" if probe_res.get("verdict") == "PASS" else "FAIL"
    final_dict["source_phase"] = "P58"
