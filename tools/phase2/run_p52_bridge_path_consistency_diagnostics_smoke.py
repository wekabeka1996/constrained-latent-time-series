...
def main():
    if len(sys.argv) > 1:
        print("Error: run_p52_bridge_path_consistency_diagnostics_smoke.py does not accept command line arguments.", file=sys.stderr)
        sys.exit(1)
    ...
    final_dict = bridge_path_consistency_diagnostics_probe_to_json_dict(probe_res)
    
    passed = bool(probe_res.get("path_consistency_passed", False))
    final_dict["verdict"] = "PASS" if passed else "FAIL"
    final_dict["source_phase"] = "P52"
