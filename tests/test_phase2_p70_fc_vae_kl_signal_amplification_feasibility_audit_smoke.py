...
def test_p70_smoke_01_run_success():
    env = os.environ.copy()
    env["PYTHONPATH"] = "."
    
    # Run the smoke tool
    res = subprocess.run(
        [sys.executable, "tools/phase2/run_p70_fc_vae_kl_signal_amplification_feasibility_audit_smoke.py"],
        capture_output=True,
        text=True,
        check=True,
        env=env
    ...
    assert data["source_phase"] == "P70"
    assert data["source_evidence_phase"] == "P69"
    assert data["status"] == "fc_vae_kl_signal_amplification_feasibility_audit_available_no_dataset_no_generalization"
    assert data["contract_version"] == "phase2_p70_fc_vae_kl_signal_amplification_feasibility_audit_contract_v1"
    
    assert data["expected_source_component_runs"] == 9
    assert data["observed_source_component_runs"] == 9
    assert len(data["kl_to_reference_ratio_diagnostics"]) == 4
    assert len(data["contribution_share_amplification_diagnostics"]) == 3
...
def test_p70_smoke_02_rejects_args():
    res = subprocess.run(
        [sys.executable, "tools/phase2/run_p70_fc_vae_kl_signal_amplification_feasibility_audit_smoke.py", "--invalid-arg"],
        capture_output=True,
        text=True
    )
