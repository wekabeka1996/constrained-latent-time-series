...
def test_p68_smoke_01_run_success():
    ...
    # Run the smoke tool
    res = subprocess.run(
        [sys.executable, "tools/phase2/run_p68_fc_vae_numeric_profile_coordinate_contribution_audit_smoke.py"],
        capture_output=True,
        text=True,
        check=True,
        env=env
    ...
    assert data["source_phase"] == "P68"
    assert data["source_evidence_phase"] == "P67"
    assert data["status"] == "fc_vae_numeric_profile_coordinate_contribution_audit_available_no_dataset_no_generalization"
    assert data["contract_version"] == "phase2_p68_fc_vae_numeric_profile_coordinate_contribution_audit_contract_v1"
    
    assert data["expected_source_component_runs"] == 9
    assert data["observed_source_component_runs"] == 9
    assert len(data["coordinate_ids"]) == 3
...
def test_p68_smoke_02_rejects_args():
    res = subprocess.run(
        [sys.executable, "tools/phase2/run_p68_fc_vae_numeric_profile_coordinate_contribution_audit_smoke.py", "--invalid-arg"],
        capture_output=True,
        text=True
    )
